...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        vector<PLL> hash_list_s = hashList(s);
        vector<PLL> hash_list_z = hashList(z);

        ll len = s.size();
        ll _max = 0;
        for (int i = len, j = 1; i >= 1; i--, j++)
        {
            PLL sub_hash_s = substringHash(hash_list_s, i, len);
            PLL sub_hash_z = substringHash(hash_list_z, 1, j);

            if (sub_hash_s == sub_hash_z)
            {
                _max = i;
            }
        }
